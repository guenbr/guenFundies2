...
public class Graph<T> {
    ...
    private class KruskalIterator implements Iterator<Edge<T>> {
        ...
        public boolean hasNext() {
            ...
            while (!pq.isEmpty() && pq.peek().getNode1().find() == pq.peek().getNode2().find()) {
                betweenNodes = pq.poll();
            }
            betweenNodes = pq.poll();
            return betweenNodes != null;
        }
        ...
        public Edge<T> next() {
            ...
            betweenNodes.getNode1().union(betweenNodes.getNode2());
            return betweenNodes;
        }

        private void concurrentModification() {
            if (myModCount != modCount) {
                throw new ConcurrentModificationException("Mod counts are not the same");
            }
        }
