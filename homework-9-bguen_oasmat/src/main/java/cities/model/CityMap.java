...
public class CityMap {
    ...
    private static final int NUM_CITIES = 50; // file has 100
    ...
    private void makeGraph() throws IOException {
        ...
        for (int i = 0; i < graph.getNodes().size(); i++) {
            for (int j = 0; j < graph.getNodes().size(); j++) {
                Graph.Node<City> node1 = graph.getNodes().get(i);
                Graph.Node<City> node2 = graph.getNodes().get(j);
                graph.addEdge(node1, node2, (int) distanceInMiles(node1.getData(), node2.getData()));
            }
        }
